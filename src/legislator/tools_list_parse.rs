use super::tools_list::ToolsListError;
...
pub fn parse_tools_list_response_page(json_str: &str) -> Result<ToolsListPage, ToolsListError> {
    let json = nojson::RawJson::parse(json_str)
        .map_err(|e| ToolsListError::ParseError(format!("invalid JSON: {e}")))?;
        ...
        && error_ref.optional().is_some()
    {
        let error_raw = json
            .value()
            .to_member("error")
            .map_err(|e| ToolsListError::ParseError(format!("error field: {e}")))?
            .optional()
            .map(|v| v.as_raw_str().to_string())
            .unwrap_or_default();
        return Err(ToolsListError::ParseError(format!(
    ...
    let result = json
        .value()
        .to_member("result")
        .map_err(|e| ToolsListError::ParseError(format!("missing 'result': {e}")))?
        .optional()
        .ok_or_else(|| ToolsListError::ParseError("'result' is null or missing".to_string()))?;
        ...
        Ok(member) => match member.optional() {
            ...
            Some(value) => Some(
                value
                    ...
                    .map_err(|e| {
                        ToolsListError::ParseError(format!("'nextCursor' is not a string: {e}"))
                    })?
                    .into_owned(),
            ),
        },
    ...
    let tools_val = result
        .to_member("tools")
        .map_err(|e| ToolsListError::ParseError(format!("missing 'tools': {e}")))?
        .optional()
        .ok_or_else(|| ToolsListError::ParseError("'tools' is null or missing".to_string()))?;
...
/// `tools_to_json`.
pub fn parse_tools_list_response(json_str: &str) -> Result<Vec<ToolDefinition>, ToolsListError> {
...
fn extract_tools(
    ...
    let tools_iter = tools_val
        .to_array()
        .map_err(|e| ToolsListError::ParseError(format!("'tools' is not an array: {e}")))?;
    ...
    for item_val in tools_iter {
        ...
        let name = item_val
            .to_member("name")
            .map_err(|e| ToolsListError::ParseError(format!("tool missing 'name': {e}")))?
            .required()
            .map_err(|e| ToolsListError::ParseError(format!("tool 'name' is null: {e}")))?
            .to_unquoted_string_str()
            .map_err(|e| ToolsListError::ParseError(format!("tool 'name' not a string: {e}")))?
            .to_string();
...
fn optional_title_string(
...
) -> Result<Option<String>, ToolsListError> {
    ...
    match member.kind() {
        nojson::JsonValueKind::String => member
            ...
            .map_err(|e| ToolsListError::ParseError(format!("tool 'title' not a string: {e}"))),
        _ => Err(ToolsListError::ParseError(
...
mod tests {
    ...
    fn test_parse_invalid_json() {
        ...
        assert!(matches!(err, ToolsListError::ParseError(_)));
    }
