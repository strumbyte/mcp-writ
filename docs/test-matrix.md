...
| `plan_report_e2e` | CI, Platform tests, Linux tests | spawns the built binary for `plan` (four fixed statuses/exit codes, no workload launch) and `run --report` (dry-run session, launch-id ↔ audit correlation, stdout stays JSON-RPC, unwritable report fails before spawn) |
