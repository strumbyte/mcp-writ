...
Under `extends`, an `environment` node declared in the extending file is authoritative: its `allow` list replaces the inherited one wholesale, even when the node declares no `allow` children — an empty `environment {}` is the supported way to revoke every inherited name. Omitting the node keeps the inherited list. Restriction itself cannot be switched back off through `extends`.
